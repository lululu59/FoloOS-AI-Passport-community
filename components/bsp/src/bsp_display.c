...
esp_err_t bsp_display_init(void) {
    ...
    /* 面板上电到首帧绘制之间可能显示控制器默认白底。先接管背光并保持
     * 关闭，等 LVGL 首帧真正送到面板后再由 main 打开，避免开机白闪。 */
    backlight_init();
...
void bsp_display_enable(bool enabled) {
    if (s_panel != NULL) esp_lcd_panel_disp_on_off(s_panel, enabled);
}
